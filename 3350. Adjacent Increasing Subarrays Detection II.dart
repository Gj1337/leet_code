...
class Solution {
  int maxIncreasingSubarrays(List<int> nums) {
    var current = 1, previous = 0, max = 1;
    ...
    for (var i = 1; i < nums.length; i++) {
      ...
      final minCommonLength = current < previous ? current : previous;
      max = minCommonLength > max ? minCommonLength : max;
      max = current ~/ 2 > max ? current ~/ 2 : max;
    }
...
class SolutionTest extends Testable<int> with ConsoleTestOutput {
  ...
  @override
  int computeResult() => Solution().maxIncreasingSubarrays(nums);
}

void main(List<String> args) {
  [
    SolutionTest(nums: [2, 5, 7, 8, 9, 2, 3, 4, 3, 1], result: 3),
    SolutionTest(nums: [1, 2, 3, 4, 4, 4, 4, 5, 6, 7], result: 2),
  ].test();
}
