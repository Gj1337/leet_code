...
// You are tasked to implement a data structure
// that supports queries of two types:
//     Add a positive integer to an element
//     of a given index in the array nums2.
//     Count the number of pairs (i, j)
//     such that nums1[i] + nums2[j] equals a given value
//     (0 <= i < nums1.length and 0 <= j < nums2.length).
// Implement the FindSumPairs class:
//     FindSumPairs(int[] nums1, int[] nums2)
//     Initializes the FindSumPairs object with
...
// // return 8; pairs (2,2), (3,2), (4,2), (2,4), (3,4), (4,4)
...
// findSumPairs.count(7);
// // return 11; pairs (2,1), (2,2), (2,4), (3,1), (3,2),
...
class FindSumPairs {
  ...
  final _freqNums2 = <int, int>{};

  FindSumPairs(this.nums1, this.nums2) {
    for (var number in nums2) {
      _freqNums2[number] = _freqNums2[number] ?? 0 + 1;
    }
  }

  void add(int index, int val) {
    final number = nums2[index];
    final newNumber = number + val;

    if (_freqNums2[number] != null) {
      _freqNums2[number] = _freqNums2[number]! - 1;
    }
    _freqNums2[newNumber] = _freqNums2[newNumber] ?? 0 + 1;
  ...
  int count(int tot) {
    ...
    for (var i = 0; i < nums1.length; i++) {
      final serchableNumber = tot - nums1[i];

      count += _freqNums2[serchableNumber] ?? 0;
    }
