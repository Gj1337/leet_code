...
class Solution {
  List<int> getSneakyNumbers(List<int> nums) {
    final uniqNumbers = <int>{};
    final result = <int>[];

    for (var num in nums) {
      uniqNumbers.contains(num) ? result.add(num) : uniqNumbers.add(num);
...
class SolutionTest extends Testable<List<int>> with ConsoleTestOutput {
  ...
  @override
  List<int> computeResult() => Solution().getSneakyNumbers(nums);
  ...
  bool validateResult(List<int> computedResult, List<int> expectedResult) {
    ...
    for (var i = 0; i < computedResult.length; i++) {
      if (computedResult[i] != expectedResult[i]) return false;
    }
...
void main(List<String> args) {
  [
    SolutionTest(nums: [0, 1, 1, 0], result: [0, 1]),
    SolutionTest(nums: [0, 3, 2, 1, 3, 2], result: [2, 3]),
    SolutionTest(nums: [7, 1, 5, 4, 3, 4, 6, 0, 9, 5, 8, 2], result: [4, 5]),
  ].test();
}
