...
// In the second operation, we remove elements 3 and 4,
// then add 3 * 2 + 4 to nums. nums becomes equal to [10, 11, 10].
...
// That is, there exists some sequence of operations
...
class Solution {
  int minOperations(List<int> nums, int k) {
    final queue = PriorityQueue<int>()..addAll(nums);

    var minOperation = 0;

    while (queue.first < k) {
      final x = queue.removeFirst();
      final y = queue.removeFirst();

      queue.add(_min(x, y) * 2 + _max(x, y));
  ...
  T _min<T extends Comparable>(T a, T b) => a.compareTo(b) <= 0 ? a : b;

  T _max<T extends Comparable>(T a, T b) => a.compareTo(b) > 0 ? a : b;
}
...
class SolutionTest extends Testable<int> with ConsoleTestOutput {
  ...
  @override
  int computeResult() => Solution().minOperations(nums, k);
}

void main(List<String> args) {
  [
    SolutionTest(nums: [2, 11, 10, 1, 3], k: 10, result: 2)
  ].test();
}
