...
// Now, nums = [4,1,4,3].
// 2. Remove 1 and 4. The average is (1 + 4) / 2 = 2.5,
// and nums = [4,3].
...
class Solution {
  int distinctAverages(List<int> nums) {
    final distinctSum = <int>{};

    nums.sort();

    for (var i = 0; i < nums.length; i++) {
      distinctSum.add(nums[i] + nums[nums.length - 1 - i]);
    }

    return distinctSum.length;
  }
}
...
class SolutionTest extends Testable<int> with ConsoleTestOutput {
  ...
  @override
  int computeResult() => Solution().distinctAverages(nums);
}

void main(List<String> args) {
  [
    SolutionTest(nums: [4, 1, 4, 0, 3, 5], result: 2),
  ].test();
}
