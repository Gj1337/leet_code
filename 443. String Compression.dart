...
// Example 2:
// Input: chars = ["a"]
// Output: Return 1, and the first character
// of the input array should be: ["a"]
  ...
  int compress(List<String> chars) {
    ...
    for (var i = 0; i < chars.length - 1; i++) {
      if (chars[i] == chars[i + 1]) {
        ...
        chars[start] = chars[i];
        if (count > 1) {
          final countNumbers = count.toString().split('');
          for (final numberChar in countNumbers) {
            start++;
            chars[start] = numberChar;
          }
        }
        start++;
        count = 1;
...
class SolutionTest extends Testable<int> with ConsoleTestOutput {
  SolutionTest({required this.chars, required super.result});

  final List<String> chars;

  @override
  int computeResult() => Solution().compress(chars);
}

void main(List<String> args) {
  [
    SolutionTest(chars: ["a", "a", "b", "b", "c", "c", "c"], result: 6),
    SolutionTest(chars: ["a"], result: 1),
    SolutionTest(chars: ["a", "a", "a", "b", "b", "a", "a"], result: 6),
    SolutionTest(chars: ["a","b","b","b","b","b","b","b","b","b","b","b","b"],
                                                                     result: 4)
  ].test();
}
