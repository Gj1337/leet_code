// There is a school that has classes of students
// and each class will be having a final exam.
// You are given a 2D integer array classes, where classes[i] = [passi, totali].
// You know beforehand that in the ith class,
// there are totali total students,
...
// to a class in a way that maximizes the average pass ratio
// across all the classes.
// The pass ratio of a class is equal to the number of students
// of the class that will pass the exam divided by the total number
...
// after assigning the extraStudents students.
...
class Solution {
  double maxAverageRatio(List<List<int>> classes, int extraStudents) {
    int compareClassesGainedRate(List<int> a, List<int> b) {
      final (aPass, aTotal) = (a.first, a.last);
      final (bPass, bTotal) = (b.first, b.last);

      final gainA = (aPass + 1) / (aTotal + 1) - aPass / aTotal;
      final gainB = (bPass + 1) / (bTotal + 1) - bPass / bTotal;

      return gainB.compareTo(gainA);
    }

    final pq = PriorityQueue<List<int>>(compareClassesGainedRate)
      ..addAll(classes);

    while (extraStudents > 0) {
      final bestGainClass = pq.removeFirst();
      bestGainClass.first++;
      bestGainClass.last++;
      pq.add(bestGainClass);
      extraStudents--;
    }

    final length = pq.length;
    final sumOfPassRatios = pq
        .removeAll()
        .map((cls) => cls[0] / cls[1])
        .reduce((sum, ratio) => sum + ratio);

    return sumOfPassRatios / length;
  }
}

class SolutionTest extends Testable<double> with ConsoleTestOutput {
  final List<List<int>> classes;
  final int extraStudents;
  ...
  double computeResult() {
    final actualResult = Solution().maxAverageRatio(classes, extraStudents);
    // Round to 5 decimal places for comparison
    return double.parse(actualResult.toStringAsFixed(5));
  }
}

void main(List<String> args) {
  [
    SolutionTest(classes: [
      [1, 2],
      [3, 5],
      [2, 2]
    ], extraStudents: 2, result: 0.78333),
    SolutionTest(classes: [
      [2, 4],
      [3, 9],
      [4, 5],
      [2, 10]
    ], extraStudents: 4, result: 0.53485)
  ].test();
}
