...
// void change(int index, int number) Fills the container
// at index with the number.
...
class NumberContainers {
  final _indexToNum = <int, int>{};
  final _numToIndexes = <int, SplayTreeSet<int>>{};

  void change(int index, int number) {
    if (_indexToNum[index] != null) {
      _numToIndexes[_indexToNum[index]]?.remove(index);
    }

    _indexToNum[index] = number;

    if (_numToIndexes[number] == null) {
      _numToIndexes[number] = SplayTreeSet<int>();
    }
  ...
  int find(int number) {
    print(_numToIndexes[number]?.firstOrNull ?? -1);

    return _numToIndexes[number]?.firstOrNull ?? -1;
  }
}
...
void main(List<String> args) {
  final nc = NumberContainers();

  nc.find(
      10); // There is no index that is filled with number 10. Therefore, we return -1.
  nc.change(2, 10); // Your container at index 2 will be filled with number 10.
  nc.change(1, 10); // Your container at index 1 will be filled with number 10.
  nc.change(3, 10); // Your container at index 3 will be filled with number 10.
  nc.change(5, 10); // Your container at index 5 will be filled with number 10.
  nc.find(
      10); // Number 10 is at the indices 1, 2, 3, and 5. Since the smallest index that is filled with 10 is 1, we return 1.
  nc.change(1,
  ...
  nc.find(
      10); // Number 10 is at the indices 2, 3, and 5. The smallest index that is filled with 10 is 2. Therefore, we return 2.
}
